...
import time
import storage
from inputctrl import InputCtrl,MainInputControl,TrainInputControl
import alarm
import board
import keypad

from screen import IdleScreen,StatusScreen,TrainScreen,BattleScreen,LightsScreen,MedicalScreen,JournalScreen,ConnectScreen,StrGameScreen,IntGameScreen,SpdGameScreen
...
class Ui:
    ...
    def __init__(self,monster):
        ...
        self.keys = keypad.Keys([board.GP15,board.GP14,board.GP13,board.GP12],value_when_pressed=False, pull=True)
    ...
    def update(self):
    	...
    	if self.currScreen.transition == "status":
            self.inputCtrl = InputCtrl(self.keys,self.inputCtrl.timeSinceInput)
        ...
        elif self.currScreen.transition == "spd-game":
            self.inputCtrl = InputCtrl(self.keys,self.inputCtrl.timeSinceInput)
            self.currScreen.destroy()
            self.currScreen = SpdGameScreen(self.monster,self.inputCtrl)
